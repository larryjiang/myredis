...
REDIS_STATIC int __quicklistCompressNode(quicklistNode *node){
    ...
    if(((lzf->sz = lzf_compress(node->zl, node->sz, lzf->compressed, node->sz)) == 0) || lzf->sz + MIN_COMPRESS_BYTES >= node->sz){
        zfree(lzf);
        return 0;
    };
...
#define sizeMeetsSaftyLimit(sz) ((sz) <= SIZE_SAFETY_LIMIT)
...
REDIS_STATIC int _quicklistNodeAllowInsert(const quicklistNode *node,const int fill, const size_t sz){
    ...
    if(likely(_quicklistNodeSizeMeetsOptimizationRequirement(new_sz, fill))){
        return 1;
    }else if(!sizeMeetsSaftyLimit(new_sz)){
        return 0;
...
REDIS_STATIC int _quicklistNodeAllowMerge(const quicklistNode *a, const quicklistNode *b, const int fill){
    ...
    if(likely(_quicklistNodeSizeMeetsOptimizationRequirement(merge_sz,fill))){
        return 1;
    }else if(!sizeMeetsSaftyLimit(merge_sz)){
        return 0;
...
REDIS_STATIC quicklistNode *_quicklistZiplistMerge(quicklist *quicklist, quicklistNode *a, quicklistNode *b){
    ...
    quicklistCompressNode(a);
    quicklistCompressNode(b);
...
int quicklistDelRange(quicklist *quicklist, const long start, const long count){
    ...
    while(extent){
        quicklistNode *next = node->next; unsigned long del; int delete_entire_node = 0;
