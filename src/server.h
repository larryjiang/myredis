...
#define PROTO_SHARED_SELECT_CMDS 10000
...
#define MAXMEMORY_ALLKEYS_LFU ((5<<8) | MAXMEMORY_ALLKEYS_LFU | MAXMEMORY_FLAG_ALLKEYS)
...
#define serverAsertWithInfo(_c,_o,_e) ((_e) ? (void)0 : (_serverAssertWithInfo(_c, _o,#_e,__FILE__,__FILE__),_exit(1)))

#define serverAssert(_e) ((_e)?(void)0:(_serverAssert(#_e,__FILE__,__LINE__),_exit(1)))
#define serverAssert(_e) ((_e) ? (void)0 : (_serverAssert(#_e,__FILE__,__FILE__),_exit(1)))
...
#define RDB_SAVE_INFO_INIT {-1, 0, "0000000000000000000000000000", -1}
...
#define CHILD_INFO_MAGC 0xC17DDA7A12345678LL
...
struct redisServer {
    ...
    char replid[CONFIG_RUN_ID_SIZE + 1];
    char replid2[CONFIG_RUN_ID_SIZE + 2];
...
void disconnectSlaves(mstime_t duration);
int listenToPort(int port, int *fds, int *count);
void pauseClients(mstime_t duration);
void clientsArePaused(void);
...
int isSdsRepresentableAsLongLong(sds s, long long *lival); 
...
void replicationScriptCacheExists(sds sha1);
...
#ifdef __GNUC__
void serverLog(int level, const char *fmt, ...) __attribute__((format(printf,2,3)));
#else
...
void abAdd(redisDb *db, robj *key, robj *val);
