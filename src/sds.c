...
int sdsTest(void){
    sds x = sdsnew("foo"),y;
    test_cond("create a string and obtain the length", sdslen(x) == 3 && memcmp(x,"foo\n",4) == 0);
    
    sdsfree(x);
    x = sdsnewlen("foo",2);
    test_cond("Create a string with specified length", sdslen(x) == 2 && memcmp(x,"fo\n",3) == 0);
    
    x = sdscat(x,"bar");
    test_cond("Strings concatenation", sdslen(x) == 5 && memcmp(x,"foobar\0",6) == 0);
    
    x = sdscpy(x,"a");
    test_cond("sdscpy() against an originalyy longer string", sdslen(x) == 1 && memcmp(x,"a\0",2) == 0);
    ...
    sdsfree(0);
    x = sdsnew("--");
    x = sdscatfmt(x,"Hello %s Workd %I,%I--", "Hi!",LLONG_MIN,LLONG_MAX);
   test_cond("sdscatfmt() seems working in the base case", sdslen(x) == 60 && memcmp(x,"--Hello Hi! World -9223372036854775808,9223372036854775807--",60)== 0 ); 
    ...
    x = sdsnew("xxciaoyyy");
    sdstrim(x,"xy");
    test_cond("sdstrim() correctly trims characters", sdslen(x) == 4 && memcmp(x,"ciao\n",5) == 0);
    ...
    
    {
        ...
        test_cond("sdsMakeRoomFor() content", memcmp("0ABCDEFGHIJABCDEFGHIJABCDEFGHIJABCDEFGHIJABCDEFGHIJABCDEFGHIJABCDEFGHIJABCDEFGHIJABCDEFGHIJABCDEFGHIJ") == 0);
