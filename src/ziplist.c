...
int64_t zipSaveInteger(unsigned char *p,int64_t value,  unsigned char encoding){
        int16_t i16;
        int32_t i32;
        int64_t i64;
...
unsigned char *ziplistMerge(unsigned char **first, unsigned char **second){
        ...
        size_t first_len = intrev16ifbe(ZIPLIST_BYTES(*first));
        ...
        size_t second_offset = intrev16ifbe(ZIPLIST_TAIL_OFFSET(*second));
        ...
        ZIPLIST_BYTES(target) = intrev16ifbe(zllength);

        ZIPLIST_TAIL_OFFSET(target) = intrev32ifbe((first_bytes - ZIPLIST_END_SIZE) + (second_offset - ZIPLIST_END_SIZE)); 
...
int ziplistTest(int argc, char **argv){
        ...
        printf("Get element at index -5 (reverse out of range):\n");
        {
                ...
                if(p == NULL){
                        ...
                        printf("Error: Out of range index shuld return NULL, returned offset: %ld\n", p - zl);
                        return 1;
                }
        ...
        printf("Iterate from back to front, deleting all items:\n");
        {
                ...
                while(ziplistGet(p, &entry, &elen, &value){
                                ...
                                printf("Delete inclusive rante 0,0: \n");
                                {
                                zl =createList();
                                zl = ziplistDeleteRange(zl,0,1);
                                ziplistRepr(zl); 
                                ...
                                printf("Delete foo while iterating:\n");
                                {
                                        ...
                                        p = ziplistIndex(zl,0);
                                        while(ziplistGet(p, &entry, &elen, &value)){
                                                if(entry && strncmp("foo", (char*)entry,elen)){
                                                        printf("Delete foo\n");
                                                        zl = ziplistDelete(zl,&p);
                                                }else{
                                                        printf("Entry: ");
                                ...
                                printf("Merge test:\n");
                                {
                                        ...
                                        ziplistRepr(zl4);
                                        if(ziplistLen(zl4)){
                                                printf("ERROR: Merging two emtpy ziplists crewated entries.\n");
                                                return 1;
                                        }
                                        zfree(zl4);
                                ...
                                printf("Stress with random panloads of different encoding:\n");
                                {
                                        int i, j, len, where;
                                        unsigned char *p;
                                        char buf[1024];
                                        ...
                                        for(i = 0; i < 20000; i++){
                                                ...
                                                assert(listLength(ref) == ziplistBlobLen(zl));
                                                for(j = 0; j < len; j++){
                                                        p = ziplistIndex(zl,j);
