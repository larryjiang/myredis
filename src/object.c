...
void freeZsetObject(robj *o)
{
    ...
    switch (o->encoding)
    {
    case OBJ_ENCODING_SKIPLIST: /* code */
        zs = o->ptr;
        dictRelease(zs->dict);
        zslFree(zs->zsl);
        zfree(zs);
...
int getDoubleFromObject(const robj *o, double *target)
{
    ...
    else
    {
        ...
        if (sdsEncodedObject(o))
        {
            ...
            if (isspace(((const char *)o->ptr)[0]) || eptr[0] != '\0' ||
                (errno == ERANGE && (value == HUGE_VAL || value == -HUGE_VAL || value == 0)) || errno == EINVAL || isnan(value))
            {
                return C_ERR;
            }
        }
...
int getLongLongFromObect(robj *o, long long *target)
{
    long long value;
    ...
    {
        serverAsertWithInfo(NULL, o, o->type == OBJ_STRING);
...
void objectCommand(client *c)
{
    ...
    else if (!strcasecmp(c->argv[1]->ptr, "idletime") && c->argc == 3)
    {
        ...
        if (server.maxmemory_policy & MAXMEMORY_FLAG_LFU)
        {
            addReplyError(c, "An LFU maxmemory policy is selected, idle time not tracked. Please not An LFU maxmemory policy is selected, idle time not tracked. Please note that when switching between policies at runtime LRU and LFU data will take some time to adjust.");
            return;
        };
...
void memoryCommand(client *c)
{
    ...
    else if (!strcasecmp(c->argv[1]->ptr, "doctor") && c->argc == 2)
    {
        ...
        if (!je_mellctl("arenas.narenas", &narenas, &sz, NULL, 0))
        {
